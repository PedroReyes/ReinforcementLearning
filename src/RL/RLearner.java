...
public class RLearner {
	...
	public int totalEpisodes; // how long do I learn? an episode is divided in steps (an episode is the number of times the game is executed)
	...
	public static enum LEARNING_PARAMETER {
		...
		epsilonSelection, c_UCBSelection, temperatureSoftmaxSelection, baselineUsedSoftmaxSelection, totalNumberStepsForSimulationInMCTS, depthCharge//
	}
	...
	public static enum SELECTION_METHOD {
		...
		, MONTE_CARLO_TREE_SEARCH // Monte Carlo Tree Search (https://jeffbradberry.com/posts/2015/09/intro-to-monte-carlo-tree-search/)
	};
	...
	// e_greedy temporally
	public double[] epsilonRange; // [initial value, final value]
	...
	// softmax
	private double temperature; // (This paramater is represented with the symbol "tao")
	...
	public int actionSelectionMethodForSimulation; // how to select the action?
	...
	public int episodeNumber; // which episode we are in
	...
	public RLearner(String simulationName, RLWorld world) {
		...
		thisWorld = world;
		stateBefore = thisWorld.getCurrentState();
		newState = thisWorld.getCurrentState();
		...
		// soft-max
		temperature = 1; // For high temperatures (\tau\to \infty), all actions have nearly the same probability
		...
		setTotalSimulatedStepsInMCTS(10);
	}
	...
	public void runTrial() {
		...
		for (taskNumber = 0; taskNumber < totalTasks; taskNumber++) {
			...
			if (false) {
				System.out.println("Last epsilon: " + lastEpsilon + " > " + Helper.getCurrentTime() + " > Task "
						+ taskNumber + " for c " + cExploratoryParameter + ", exploratory value " + epsilon + ", alpha "
						+ alpha + ", and gamma " + gamma);
			}

			//			System.out.println("Episodes executed:");
			// Start the task (composed by several episodes)
			for (episodeNumber = 0; episodeNumber < totalEpisodes; episodeNumber++) {
				//				System.out.print("("+Helper.getCurrentTime() + " > "+"Episode " + episodeNumber+")");
				// Start the episode (composed by several episode steps)
				//				System.out.println("Episode "+episodeNumber +": " +epsilon);
				if (true) {
					System.out.println("Task " + taskNumber + " - Episode " + episodeNumber + " - " + "epsilon: "
							+ epsilon + " with " + timeStep + " steps. " + "Reward: " + reward);
				}
				runEpoch();
			}
	...
	public int selectAction(State state) {
		...
		case MONTE_CARLO_TREE_SEARCH: {
			selectedAction = mctsSelection(state);
			break;
		}
		default: {
	 ...
	 * @return
	 */
	public int eGreedySelection(State state) {
		// ==============================
		thisWorld.setWorldState(state);
		// ==============================
	 ...
	 * @return
	 */
	public int ucbSelection(State state) {
		// TODO Auto-generated method stub
		// ==============================
		thisWorld.setWorldState(state);
		// ==============================
		...
		double[] qValuesWithBound = new double[qValues.length];
		double[] bound = new double[qValues.length];
		...
		for (int action = 0; action < qValues.length; action++) {
			...
			if (thisWorld.validAction(action)) {
				...
				bound[action] = cExploratoryParameter
						* (Math.sqrt((Math.log(timeStep) / timesSelected[action][ordinalState])));
				//			qActionValue = (double) Math.round(qActionValue * decimals) / decimals;
				qValuesWithBound[action] = qActionValue;
	 ...
	 * @return
	 */
	public int mctsSelection(State state) {
		Integer selectedAction = 0;

		selectedAction = mcts.runMonteCarloTreeSearch();
	 ...
	 * @return
	 */
	public int softMaxSelection(State state) {
		// ==============================
		thisWorld.setWorldState(state);
		// ==============================		
	 ...
	 * each action using the softmax function.
	 *  
	 * @param qValues
	 * @return
	 */
	private double[] actionProbabilities(double[] qValues) {
		...
		for (action = 0; action < qValues.length; action++) {
			prob[action] = Math.exp(qValues[action] / temperature);
			sumProb += prob[action];
		}
		for (action = 0; action < qValues.length; action++)
	...
	public RLPolicy newPolicy() {
		...
		if (true) {
			for (int i = 0; i < timesSelected.length; i++) {
				for (int j = 0; j < timesSelected[0].length; j++) {
					timesSelected[i][j] = 0;
				}
			}
		}
	...
	public double getRootMeanSquareErrorFromCurrentPolicy() {
		...
		RealMatrix matrixOfEstimatePolicy = policy.getqValuesTable();
	...
	public String toString() {
		return "RLearner [simulationName=" + simulationName + ", thisWorld=" + thisWorld + ", \npolicy=" + policy
				...
				+ ", timesSelected=" + Arrays.toString(timesSelected) + ", temperature=" + temperature
	...
	public void setActionSelectionMethod(SELECTION_METHOD as) {
		...
		case SOFT_MAX:
			...
			break;
		case MONTE_CARLO_TREE_SEARCH:
			actionSelectionMethod = SELECTION_METHOD.MONTE_CARLO_TREE_SEARCH.ordinal();
			break;
		default:
	...
	public void setTotalSimulatedStepsInMCTS(int totalSteps) {
		if (mcts == null) {
			// MCTS
			mcts = new MonteCarloTreeSearch(this);
		}
		mcts.totalNumberSteps = totalSteps;
	}

	public int getActionSelectionMethodForSimulation() {
		return actionSelectionMethodForSimulation;
	}
