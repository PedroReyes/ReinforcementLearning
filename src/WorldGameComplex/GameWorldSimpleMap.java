...
public class GameWorldSimpleMap implements RLWorld {
	...
	public State getNextState(int action) {
		...
		if (enemyLife <= thresholdEnemyLifePercentageForLaunchingAirAttack * enemyFullLife
			...
			if (false) {
				for (int i = 0; i < w; i++) {
					for (int j = 0; j < h; j++) {
						gameMap.getMap().put(new Point(i, j), STATES.ZONA_EMBOSCADA);
					}
				}

				// ... except for ...
				gameMap.getMap().put(new Point(0, h), STATES.ZONA_ANTIBOMBARDEOS);
			}
		} else {
			RAA = 0;
			if (false) {
		...
		// Calculate reward (totally based on where the agent is)
		//		calculateRewardUsingLife();
		calculateRewardUsingStates();

		if (stepOfTheGame < 100 && false) {
			System.out.println("AgentLife(" + agentLife + ")" + ", EnemyLife(" + enemyLife + ")" + ", BombDamage("
					+ bombDamagePerStep + ") > " + stateName + " --> R:" + waitingReward);
		}
	...
	/**
	 * Calculate the reward being in the current state using life information
	 */
	private void calculateRewardUsingLife() {
		// =============================================================
		...
		if (currentState.getState()[STATES.ZONA_ATAQUE.ordinal()] == 1) { // ZONA DE ATAQUE
			// Benefits and costs
			double beneficio = (enemyFullLife - enemyLife) / enemyFullLife;
			double coste = (agentFullLife - agentLife) / agentFullLife;

			// REWARDS (Pedro)
			//waitingReward = enemyLifeDiferential / agentLife;
			waitingReward = (agentLife == 0 && enemyLife == 0) ? -1 : beneficio - coste;
			...
			if (currentState.getShortDescription().contains(STATES.RECIBIENDO_ATAQUE_AEREO.name())) {
				coste = currentState.getShortDescription().contains(STATES.RECIBIENDO_ATAQUE_AEREO.name())
						? (agentFullLife - agentLife) / agentFullLife : 0;
			} else {
				beneficio = currentState.getShortDescription().contains(STATES.RECIBIENDO_ATAQUE_AEREO.name()) ? 0
						: (agentFullLife - agentLife) / agentFullLife;

			}

			// REWARDS (Pedro)
			waitingReward = beneficio - coste;
	 ...
	 * @return
	 */
	public static String generateStateName(double[] auxState) {
		String stateName = "";
		...
		// COORDINATES
		stateName = stateName + auxState[numberOfStates];
	...
	// ===================================
	public static Dimension getCoords(ACTIONS action, Point coordinates) {
	...
	public State getNextState(State state, int action) {
		...
		if (false) {
			System.out.println("2_Action: " + ACTIONS.values()[action]);
			System.out.println("Life: " + Arrays.toString(getCurrentLifes()));
	...
	public void setWorldState(State state) {
		...
		//		calculateRewardUsingStates(); // waitingReward = 0;
	}
	...
	public Action[] findLegalActions(State state) {
		...
		// Setting the state in the instance of the world
		setWorldState(state);

		// Getting legal actions for current state
		List<Action> legalActions = new LinkedList<>();
		for (int i = 0; i < actions.length; i++) {
