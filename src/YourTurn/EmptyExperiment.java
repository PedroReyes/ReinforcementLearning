...
public class EmptyExperiment {
	...
	public static void main(String[] args) {
		...
		int depthCharge = 3; // If depth charge is 0, then the algorithm goes to the end of the game in the simulation playouts (taking more time)
		...
		if (ejecutarExperimento) {
			experimentsExecution(tStart, urlDelExperimento,
					...
					discountFactorValues, indexCombination, nombreParametros);
		}
	...
	private static void experimentsExecution(long tStart, String urlDelExperimento,
			...
			double[] lambdaValues, int[] indexCombination, String[] nombreParametros) {
		// ================================
		...
		for (int iterations = 0; iterations < numberOfParameterCombinations; iterations++) {
			...
			// THE WORLD
			// ================================
			RLWorld world = new EmptyModel(null, null, new GameMap());

			// ================================
			// SETTING THE NAME OF THE FILE THAT IS CREATED
			// ================================
			double epsilonFinal = 0.2; // solo en caso de un epsilon temporal
